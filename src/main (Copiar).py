...
def run_with_sudo(cmd, password):
    """
    Ejecuta un comando con sudo usando el password proporcionado.
    """
...
def main(page: ft.Page):
    ...
    password_error = ft.Text(value="", color=ft.Colors.RED_400)
    ...
    def on_password_submit(e):
        ...
        # Ejemplo: actualizar la base de datos de paquetes
        code, out, err = run_with_sudo(['pacman', '-Sy'], password)
        if code == 0:
            output_text.value = out
            password_error.value = ""
            close_dialog(page, dlg_password)
        else:
            output_text.value = ""
            password_error.value = "Incorrect password or command failed."
        page.update()

    dlg_password = ft.AlertDialog(
        modal=True,
        title=ft.Text("Superuser Password"),
        content=ft.Column([
            password_field,
            password_error
        ]),
        actions=[
            ft.TextButton(text="Accept", on_click=on_password_submit),
            ft.TextButton(text="Cancel", on_click=lambda e: close_dialog(page, dlg_password)),
        ],
    ...
    def close_dlg_editor_and_install(e):
        ...
        print("Editor dialog closed and install")
        install_editors(e)
        page.update()
    ...
    def close_dlg_editor(e):
        dlg_editor.open = False
        print("Editor dialog closed")
        page.update()

    # Dialog to open dialog
    def open_dlg_editor(e):
        if dlg_editor not in page.overlay:
    ...
    def close_dlg_tecn_and_install(e):
        dlg_tecn.open = False
        print("Technologies dialog closed")
    ...
    def open_dlg_tecn(e):
        if dlg_tecn not in page.overlay:
            page.overlay.append(dlg_tecn)
        dlg_tecn.open = True
        print("Technologies dialog opened")
        page.update()
    ...
    def close_dlg_other_and_install(e):
        ...
        print("Other dialog closed and install")
        install_other(e)
        page.update()

    def close_dlg_other(e):
        dlg_other.open = False
        print("Other dialog closed")
        page.update()

    def open_dlg_other(e):
        if dlg_other not in page.overlay:
            page.overlay.append(dlg_other)
        dlg_other.open = True
        print("Other dialog opened")
        page.update()
    ...
    def open_dlg_yay():
        if dlg_install_yay not in page.overlay:
            page.overlay.append(dlg_install_yay)
        dlg_install_yay.open = True
        print("Yay dialog opened")
        page.update()

    ####### Dialog INSTALLED #######

    def open_dlg_installed():
        if dlg_installed not in page.overlay:
            page.overlay.append(dlg_installed)
        dlg_installed.open = True
        print("Installed dialog open")
        page.update()

    def close_dlg_installed(e):
        dlg_installed.open = False
        print("Installed dialog closed")
        page.update()
